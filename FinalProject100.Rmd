...
data <- read.csv("~/Downloads/PSTAT 100/DataScience_salaries_2024.csv")
...
knitr::kable(head(data_no_outliers, 10), caption = "Data Science Salaries (clean)")
```

Through EDA, we made several observations. First, we had initially attempted to create a map illustrating how `company_location` influenced `salary_in_usd`. However, there are too few observations in countries outside the US relative to the observations in which `company_location` is in the US. Thus, we decided to filter our data so the observations in our data set are focused on US companies only. Second, we noticed a very interested pattern when plotting the average salaries over time. We noticed that there was a sudden drop in average salary from 2020 to 2021, which we attributed to the Covid-19 pandemic, and then a increase jump in salary from 2022 to 2023, which we attributed to the rise in popularity of generative AI. However, despite the fact that the data science industry has taken huge leaps in the past year, we were curious as to why there is a *decrease* in average salary from 2023 to 2024. This will be looked into later on in this study. Finally, we created a word cloud diagram comparing the most common job titles associated with a lower salary (< Q1) and those associated with a higher salary (> Q3). After filtering out the word "data", we found that roles such as "analyst", "business intelligence", and even "scientist" receive lower salaries than roles such as "machine learning", or "research." Interestingly, "engineer" appeared in the lower salary range almost equally as frequent as it did in the higher salary range. Thus, using this initial analysis, we wanted to further explore the relationship between `salary_in_usd`, `job_title`, and `work_year`.\
...
## Modeling Process
Our initial thought-process was that we should start out with a simple or multiple linear regression model. However, we noticed that we were consistently getting very low $R^2$ values that only explained sub 20% of the variance observed in `salary_in_usd`, suggesting that the data is non-linear even after log-transforming the variables. Though the histogram of the residuals showed a nearly symmetrical bell-curve shape, there was a slight right skew. This observation paired with the fact that the data in the QQ-plot of the residuals showed noticeable deviations from the trend line suggested that the residuals did not follow a normal distribution (see appendix). As confirmation that the assumptions to use a linear regression model had not been met, the plot of the residuals against the fitted values showed a cone-line shape expanding along the range of fitted values, showing heteroscedasticity (see Figure 2). Thus, the assumptions to perform linear regression have not been met.\
...
p1 <- ggplot(data.frame(Predictions = predictions, Residuals = residuals), 
...
year = test$work_year
p2 <- ggplot(data.frame(Actual = test$salary_in_usd, Predicted = predictions), 
...
Back to our initial question: has the growth of AI influenced job titles and salary trends in the data science field? The answer is both yes and no. Our initial hypothesis was only partially correct in that AI-specific roles tend to earn a higher salary than traditional (or "Other") roles. However, by Figure 11, it does not appear as if the demand for AI-specific roles has increased more significantly than the demand for traditional data science roles. This was a surprising revelation considering the recent boom in the AI industry.\
